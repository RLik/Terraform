...
export IMAGES_PATH="/images" # defines the path where the images will be stored and change the path to it.
...
export VM_STORAGE_NAME="SSD_NVMe512"
...
# Import Disk
qm set ${TEMPLATE_ID} --scsi0 ${VM_STORAGE_NAME}:0,import-from=${VM_DISK_IMAGE}

# Add Cloud-Init CD-ROM drive. This enables the VM to receive customization instructions during boot.
qm set ${TEMPLATE_ID} --ide2 ${VM_STORAGE_NAME}:cloudinit --boot order=scsi0
